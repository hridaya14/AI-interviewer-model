...
class VideoModel:
    ...
    def predict_traits(self, x_features):
        '''
        Predicts the big five personality traits and interview readiness from the extracted features.
        
        Parameters:
            - x_features (np.array): extracted features from the video
        
        Returns:
            - dict: predicted trait values and interview readiness score
        '''
        try:
            if x_features is not None:
                # predicting big 5 traits
                trait_preds = {trait: self.trait_model[trait].predict(x_features.reshape(1, -1))[0] for trait in trait_names}
                
                # preparing input for random forest
                trait_array = np.array([trait_preds[trait] for trait in trait_names])
                rf_input_new = np.concatenate([x_features.reshape(1, -1), trait_array.reshape(1, -1)], axis=1)
                
                #predicting interview readiness
                interview_readiness = self.rf_model.predict(rf_input_new)[0]
